	...
	let settings = {
		...
		rules: {
			...
			chalk: {
				...
				tidy(token) {
					if (token.isClosing == '/') {
						token.content = chalk.reset._styler.open;
						...
						// console.log('SPEW', chalkFunc);
						if (!chalkFunc?._styler?.open) throw new Error(`Invalid color return for ${colors.join('+')}`);
						token.content = chalkFunc._styler.open;
					}
				},
				operate({content}) {
					return content;
