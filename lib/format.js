			...
			if (match = /^\[(?<token>.+?)\]/.exec(examine)) { // Start next token group
				...
				if (token == 'list') { // {{{
					...
					let callContext = {
						...
						findLastStackNumber() {
							return findLast(stack, s => s.type == 'number');
						},
						replaceLastStackNumber(func) {
							let stackItem = callContext.findLastStackNumber();
							if (!stackItem) throw new Error('Cannot find nearest stack number');
							stackItem.content = func(stackItem.content);
						},
					};
